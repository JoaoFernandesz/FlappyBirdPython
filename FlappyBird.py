...
pygame.font.init()
FONTE_PONTOS = pygame.font.SysFont('arial', 58)
...
class Passaro:
    ...
    def desenhar(self,tela):
        # definir a imagem do passaro que vamos usar
        ...
        # se o passaro tiver caindo nao bater asa
        if self.angulo <= -80:
            self.imagem = self.IMGS[1]
            self.contagem_imagem = self.TEMPO_ANIMACAO * 2

        # desenhar imagem
        imagem_rolacionada = pygame.transform.rotate(self.imagem,self.angulo)
        posicao_centro_imagem = self.imagem.get_rect(topleft=(self.x, self.y)).center
        retangulo = imagem_rolacionada.get_rect(center=posicao_centro_imagem)
        tela.blit(imagem_rolacionada, retangulo.topleft)
